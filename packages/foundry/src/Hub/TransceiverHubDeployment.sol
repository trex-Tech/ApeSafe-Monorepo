...
abstract contract TransceiverHubDeployment {
    function _intiateTransceiverHubDeployment(
        address _nttProxy,
        bytes memory transceiverBytecode,
        IWormholeDeployment.DeploymentParams memory deploymentParams
    ) internal returns (address) {
        bytes memory tConstructorArgs = abi.encode(
            _nttProxy,
            deploymentParams.wormholeCoreBridge,
            deploymentParams.wormholeRelayerAddr,
            deploymentParams.specialRelayerAddr,
            202,
            100000
        );

        bytes memory bytecodeWithArgs = abi.encodePacked(transceiverBytecode, tConstructorArgs);

        bytes32 salt = keccak256(abi.encodePacked(_nttProxy, "NttFactory"));

        address _transceiver = Create2.deploy(0, salt, bytecodeWithArgs);

        WormholeTransceiver transceiverProxy = WormholeTransceiver(address(new ERC1967Proxy(_transceiver, "")));
